...
if [ -n "$TAILSCALE_IP" ]; then
    ...
    echo "   📱 Unified Interface:    http://$TAILSCALE_IP:7862 ⭐ (automatic pipeline)"
    echo "   🎭 2mv Shape Only:       http://$TAILSCALE_IP:7861 (multiview shapes)"
    echo "   🎨 2.1 PBR Only:        http://$TAILSCALE_IP:7860 (texture generation)"
    ...
    echo "   ssh -L 7862:localhost:7862 -L 7861:localhost:7861 -L 7860:localhost:7860 ubuntu@$PUBLIC_IP"
    echo "   📱 Unified Interface: http://localhost:7862 ⭐"
    echo "   🎭 2mv Shape Only:    http://localhost:7861"
    echo "   🎨 2.1 PBR Only:     http://localhost:7860"
fi

echo ""
echo "🎭 Pipeline Components Installed:"
echo "   - Hunyuan3D-2mv: Shape generation (6GB VRAM) → Port 7861"
echo "   - Hunyuan3D-2.1: PBR texturing (21GB VRAM) → Port 7860"
echo "   - Unified interface: Automatic workflow → Port 7862"

echo ""
echo "🔧 Manual Interface Startup:"
echo "   # Start unified hybrid interface (port 7862):"
echo "   GRADIO_HOST=0.0.0.0 GRADIO_PORT=7862 $HYBRID_DIR/run_hybrid.sh"
echo ""
echo "   # Start 2mv multiview interface (port 7861):"
echo "   cd ~/Hunyuan3D-2mv && GRADIO_HOST=0.0.0.0 GRADIO_PORT=7861 ./run_hunyuan3d_mv.sh"
echo ""
echo "   # Start 2.1 PBR interface (port 7860):"
echo "   cd ~/Hunyuan3D-2.1 && GRADIO_HOST=0.0.0.0 GRADIO_PORT=7860 ./run_hunyuan3d.sh"
echo ""
if [ -n "$TAILSCALE_IP" ]; then
...
echo "   1. 📱 Unified (7862): Upload → automatic 2mv→2.1 pipeline → download result"
echo "   2. 🔧 Manual: Generate shape (7861) → texture mesh (7860) → download result"
echo "   3. 🎭 Shape-only (7861): Quick multiview shape generation"
echo ""
echo "💡 Tip: If unified interface has issues, use individual interfaces on ports 7860 & 7861"
echo ""
